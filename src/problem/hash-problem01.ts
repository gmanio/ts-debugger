...
function solution(participant: string[], completion: string[]) {
  const noCompletion = participant.filter(x => {
    const completionIndex = completion.indexOf(x);
  ...
  return noCompletion.length === 1 ? noCompletion.pop() : noCompletion;
}
...
const result = solution(participant3, completion3);
